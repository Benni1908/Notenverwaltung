...
c.execute("""CREATE TABLE IF NOT EXISTS notenliste (
            Vorname text,
            Nachname text,
            Matrnr text
            )""")
...
def close():
    answer = tkinter.messagebox.askquestion('Wirklich?', 'Willst du wirklich das Programm beenden? :(')
    if answer == 'yes':
        root.quit()
...
subMenu.add_checkbutton(label='check Test')

editMenu = Menu(menu)
menu.add_cascade(label="Test", menu=editMenu)
editMenu.add_command(label="Test", command=doNothing)
...
class Main:
    ...
    def insert_stud(self):
        with conn:
            ...
            logger.log(logging.ERROR, self.nachname + ', ' + self.vorname + ' (' + self.matrnr + ') wurde gespeichert')
    ...
    def remove_stud(self):
        with conn:
            ...
            self.matrnr = self.matrnr_entry.get()
            # if self.matrnr == "":
            #     answer = tkinter.messagebox.askquestion('Wirklich?', 'Willst du wirklich ' + self.matrnr + 'löschen?')
            # else:
            #     answer = tkinter.messagebox.askquestion('Wirklich?', 'Willst du wirklich ' + self.getName() + 'löschen?')
            answer = tkinter.messagebox.askquestion('Wirklich?', 'Willst du wirklich ' + self.nachname + ', ' + self.vorname + ' (' + self.matrnr + ') löschen?')
            if answer == 'yes':
    ...
    def search(self):
        if self.nachname_entry.get() == "":
            logger.error('Bitte einen Nachnamen eintragen!')
        else:
            studs = self.get_studs_by_name(self.nachname_entry.get())
            logger.warning(studs)

    def printName(self):
        print("Test!")
        logger.log(logging.ERROR, main1.getName())
    ...
    def getName(self):
        asdf = self.vorname_entry.get() + ' ' + self.nachname_entry.get()
        return asdf


# studs = get_studs_by_name('Eli')
# print(studs)
...
status = Label(main1.frame, text="Ausgewählte Tabelle: " + table_name, bd=1, relief=SUNKEN, anchor=W)
