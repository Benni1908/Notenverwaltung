...
class FormUi:
    ...
    def submit_message(self):
        ...
        # lvl = logging.level.get()
        logger.log(lvl, self.message.get())
        logger.log(logging.ERROR, 'hi')
...
class ThirdUi:
    ...
    def __init__(self, frame):
        self.frame = frame
        label31 = ttk.Label(self.frame, text='This is just an example of a third frame')
        label31.grid(column=0, row=0, sticky=W)
        label32 = ttk.Label(self.frame, text='With another line here!')
        label32.grid(column=0, row=4, sticky=W)
...
# Initialize all frames
form = FormUi(form_frame)
console = ConsoleUi(console_frame)
third = ThirdUi(third_frame)
# clock = Clock()
# clock.start()
